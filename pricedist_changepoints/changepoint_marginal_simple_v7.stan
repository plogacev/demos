functions {
  ...
  real compute_marginal_loglik_path(int t1, int t2, vector marginal_loglik, vector lp_cp, vector lp_ncp, matrix segments_loglik, vector lp_change_prior)
  {
      ...
      return prev_marginal + lp_path + ll_segment + lp_segment_change;
  }
  ...
  real compute_marginal_loglik(vector lp_cp, vector lp_ncp, matrix segments_loglik, vector lp_change_prior)
  {
        ...
        for (t2 in 1:n_time_points) {
            ...
            for (t1 in 1:t2) {
                ...
                path_lls[t1] = compute_marginal_loglik_path(t1, t2, marginal_loglik, lp_cp, lp_ncp, segments_loglik, lp_change_prior);
            }
    
            // Aggregate over paths to compute marginal
            marginal_loglik[t2+1] = log_sum_exp(path_lls);
        }
