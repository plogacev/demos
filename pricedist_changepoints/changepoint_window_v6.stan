functions {
  ...
  real delta_histogram_mean( vector left_segment_histogram, vector right_segment_histogram, vector price_points)
  {
...
parameters {
    ...
    real<lower=0.5> cp_mean;
    real<lower=0> cp_sigma;
...
model {
  ...
  for (t in 1:n_time_points) {
      ...
      if (t < n_time_points) {
          ...
          real delta = delta_histogram_mean( left_segment_histogram, right_segment_histogram, price_points);
