...
Each day is represented by a histogram over discretize price points, and the time series is transformed into a histogram-valued sequence. Changepoints are defined as time indices where the underlying price distribution regime changes.

We define the likelihood of a segmentation as the sum of within-segment log-likelihoods, each modeled as a multinomial distribution over price bins. To regularize the solution and avoid overfitting, we place a Bernoulli prior on each possible changepoint with success probability λ. The log-posterior of a changepoint configuration is:

$$
\text{log}~p(data∣z) + \text{log}~p(z)
$$
...
## All core logic for changepoint selection is implemented in C++ via Rcpp, ensuring fast evaluation of segment likelihoods.

- All core logic for changepoint selection is implemented in C++ via Rcpp, ensuring fast evaluation of segment likelihoods.

- Data generation mimics realistic pricing regimes, including list price changes, discount availability, and variation in discount probability.

- The method is visualized by overlaying detected changepoints on scatter plots of price over time, providing intuitive insight into model performance.

## Future Work

- In the current implementation, changepoints are estimated via maximum a posteriori (MAP) using a greedy search.

- In the future, we aim to implement sampling-based inference (e.g., Metropolis-Hastings or Sequential Monte Carlo) to approximate the full posterior distribution over changepoint configurations, enabling uncertainty quantification and model averaging.
...
The core of the model is the log-likelihood of a price histogram segment. For a time window $[t_1, t_2]$, we compute the (regularized) empirical histogram:
...
# Visualize the synthetic price distributions over time. Transparency reflects 
# quantity sold; red dashed lines denote predefined changepoints.
