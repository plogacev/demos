functions {
  ...
  vector extract_segment_histogram(int t_start, int t_end, matrix histogram_cumulative)
  {
      ...
      end_vals = histogram_cumulative[t_end];
  ...
  vector compute_lp_change_prior(vector change_magnitude, real prior_mu, real prior_sigma, real prior_lp_default)
  {
      ...
      for (t in 1:T) {
          // lp[t] = lognormal_lpdf( fmax(change_magnitude[t], epsilon) | prior_mu, prior_sigma);
...
parameters {
  ...
  real<lower=0> prior_change_sigma;
  real<upper=0> prior_lp_default;  
}
...
transformed parameters {
  ...
  vector[n_time_points] lp_change_prior = compute_lp_change_prior(change_magnitudes, prior_change_mu, prior_change_sigma, prior_lp_default);
}
