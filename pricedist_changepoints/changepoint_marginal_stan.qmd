...
This model estimates changepoints in time-series histogram data by marginalizing over all possible segmentations using a Bayesian framework.
The posterior has three main components, each of which is described below.
...
These are stored in a matrix of size $T \times T$, and computed from:
- Poisson or multinomial models of the observed histograms
- Or another domain-specific scoring rule
...
data_stan_marginal <- list(
  ...
  histogram = histogram_qty[1:n,],
  price_points = price_points,
  change_window_size = 50,
  prior_cp_probs_one = 2,
  prior_min_change_mu = .7,
  prior_change_mu_center = 1
)
...
model <- cmdstan_model("./changepoint_marginal_simple_v6.stan")
...
opt$summary( c("cp_probs_one", "prior_change_mu", "prior_change_sigma") )
# non_cp_prob <- opt$summary("non_cp_prob")
# matrix(non_cp_prob$estimate, ncol = n, nrow = n) %>% round(2)

```

```{r fig.height=3, fig.width=10}
cp_probs <- opt$summary("lp_cp") %>% .$estimate %>% exp()
#deltas <- opt$summary("deltas") %>% .$estimate

data.frame( idx = seq_along(cp_probs), cp_probs = cp_probs ) %>% #, delta = deltas 
  #pivot_longer(cp_probs:delta) %>% 
