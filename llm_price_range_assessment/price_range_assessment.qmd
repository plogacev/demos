...
if False:
  ...
  def summarize_mt5(title, description, lang="es", max_tokens=128):
      prompt = f"""
...
#@memory.cache(ignore=["client"], verbose=10)
#def summarize_product_descriptions_batch(client, product_list):
#  print("called")
#  return {}
```
...
if False:
  summarize_product_descriptions(client, product_list[2660:2670], batch_size = 10)
  
  x1 = summarize_product_descriptions_batch(client, product_list[2660:2665])
  x1 = re.sub(r"^```(?:python)?\s*|\s*```$", "", x1.strip(), flags=re.IGNORECASE)
  pprint.pprint( ast.literal_eval(x1) )
  
  x2 = summarize_product_descriptions_batch(client, product_list[2665:2670])
  print(x)
  
  import pprint
  pprint.pprint(x)
  
  x = re.sub(r"^```(?:python)?\s*|\s*```$", "", x.strip(), flags=re.IGNORECASE)
  ast.literal_eval(x)
...
product_list[2660:2669]

```

```{python}

pprint.pprint(product_summaries)
pl.DataFrame(product_summaries[19:21])

pprint.pprint(product_summaries[19:21])
...
#If you cannot find enough relevant price data or the product is not typically sold online in this region, return:
#[product_id, None, None, None]
...
def retrieve_price_ranges(client, product_list, currency, year, region, batch_size=10):
    """ """
    num_batches = math.ceil(len(product_list) / batch_size)
    product_list_it = iter(product_list)
    ...
    for _ in tqdm(range(num_batches), desc="Retrieving price ranges"):
        product_batch = list(itertools.islice(product_list_it, batch_size))
        product_summary = summarize_product_descriptions(client, product_batch, model="gpt-3.5-turbo")
        print(product_summary)
        result_batch = retrieve_price_ranges_batch(client, product_summary, currency, year, region)
        results.extend(result_batch)
...
cur_product_list_complete = products[i_start:i_end].to_dicts()
cur_product_list = products[['product_id', 'product_title', 'product_description']][i_start:i_end].to_dicts()
len(cur_product_list)

estimates = retrieve_price_ranges(client, cur_product_list, currency, year, region, batch_size = batch_size)
